...
from django.db.models import Index, F, ExpressionWrapper, FloatField, Avg
...
class MonthlyRate(models.Model):
    ...
    def calculate_average_rate(cls, currency_id, year):
        """Equivalent to stored function using Django aggregation"""
...
# VIEW equivalent using Django Manager with custom queryset
class ExchangeRateManager(models.Manager):
    def get_summary_view(self):
        """Equivalent to SQL VIEW using Django ORM"""
        from django.db.models import Case, When, Value, CharField
        
        return MonthlyRate.objects.select_related('currency').annotate(
...
# TRIGGER equivalent using Django signals
from django.db.models.signals import post_save, pre_save
from django.dispatch import receiver

class CurrencyRateAudit(models.Model):
    currency = models.ForeignKey(Currency, on_delete=models.CASCADE, related_name='audit_logs')
    year = models.IntegerField()
    ...
    class Meta:
        indexes = [
            models.Index(fields=['currency', 'updated_at'], name='idx_audit_currency_date'),
        ]
        ordering = ['-updated_at']
    
    def __str__(self):
        return f"Audit: {self.currency.COUNTRY} - {self.year}-{self.month}"

# TRIGGER implementation using signals
@receiver(pre_save, sender=MonthlyRate)
def capture_old_rate(sender, instance, **kwargs):
    """Trigger: Capture old rate before update"""
    if instance.pk:
        try:
...
def create_audit_log(sender, instance, created, **kwargs):
    """Trigger: Create audit log after save"""
...
# STORED PROCEDURE equivalent using Django model method
class RateManager(models.Manager):
    def update_rate_procedure(self, currency_id, year, month, rate):
        """Equivalent to stored procedure with validation"""
